...
/**
 * @title Gupta Token Protocol
 * @notice Mint NFTs with burnable conditions
 */

contract GuptaNFT is ERC1155PresetMinterPauserUpgradeable {
    ...
    IERC20Upgradeable public gupc;

    uint256 public ethFee;
    uint256 public gupcFee;
    ...
    /**
     * @notice Create NFT Collecions paying with GUPC
     */
    function buyWithGUPC(
        uint256 amountTokens,
        address tokenAddress,
        uint256 minBalance,
        uint256 expiration,
        ...
        // uint256 amountFee = gupcFee.mul(amountTokens);

        uint256 amountFee = gupcFee;

        // Collect fees in GUPC token
        gupc.safeTransferFrom(_msgSender(), feeRecipient, amountFee);
    ...
    function setGUPCFee(uint256 _gupcFee) external onlyAdmin {
        gupcFee = _gupcFee;
    }
